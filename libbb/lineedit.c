...
static struct lineedit_statics *lineedit_ptr_to_statics;
...
#define INIT_S() do { \
	lineedit_ptr_to_statics = xzalloc(sizeof(S)); \
	barrier(); \
	cmdedit_termw = 80; \
...
int FAST_FUNC read_line_input(line_input_t *st, const char *prompt, char *command, int maxsize, int timeout)
{
	...
	if (tcgetattr(STDIN_FILENO, &initial_settings) < 0 /* || !(initial_settings.c_lflag & ECHO  )*/) {
...
#define command command_must_not_be_used
	...
	if(tcsetattr_stdin_TCSANOW(&new_settings) < 0) {
		//NSLog(@"[read_line_input] tcsetattr failed: %s", strerror(errno));
	}
...
//			}
		...
		//NSLog(@"[read_line_input] switch done");
		if (break_out)
			break;
