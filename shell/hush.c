...
/* Build knobs */
#define ENABLE_HUSH_INTERACTIVE 1 // XXX
#undef ENABLE_FEATURE_SH_NOFORK
#define ENABLE_FEATURE_SH_NOFORK 1
#define LEAK_HUNTING 0
#define BUILD_AS_NOMMU 0

#undef APPLET_IS_NOFORK
#define APPLET_IS_NOFORK(a) (1)

#undef APPLET_IS_NOEXEC
#define APPLET_IS_NOEXEC(a) (1)
...
static void sigexit(int sig)
{
	...
	if (G_saved_tty_pgrp && getpid() == G.root_pid) {
		...
		// XXX tcsetpgrp(G_interactive_fd, G_saved_tty_pgrp);
	}
...
static int FAST_FUNC file_get(struct in_str *i)
{
 ...
 take_cached:
		...
		if (G_interactive_fd && i->file == stdin) {
			do {
				////NSLog((@"[hush] get_user_input");
				get_user_input(i);
				////NSLog((@"[hush] get_user_input returned");
...
#endif
				debug_leave();
				////NSLog((@"parse_stream return %p: ""end_trigger char found\n",ctx.list_head);
...
# endif
 ...
 new_cmd:
		...
		default:
			if (HUSH_DEBUG) {
				//NSLog(@"BUG: unexpected %c\n", ch);
			}
		}
	} /* while (1) */

 parse_error:
 	////NSLog((@"[hush] parse_error");
	{
...
static void parse_and_run_stream(struct in_str *inp, int end_trigger)
{
	...
	while (1) {
		...
		//////NSLog((@"[hush] debug_print_tree");
		debug_print_tree(pipe_list, 0);
		//////NSLog((@"[hush] debug_printf_exec");
		debug_printf_exec("parse_and_run_stream: run_and_free_list\n");
		//////NSLog((@"[hush] run_and_free_list");
		run_and_free_list(pipe_list);
		//////NSLog((@"[hush] done here");
		empty = 0;
#if ENABLE_HUSH_FUNCTIONS
...
{
	struct in_str input;
	////NSLog((@"[hush] setup_file_in_str");
	setup_file_in_str(&input, f);
	////NSLog((@"[hush] parse_and_run_stream");
...
# endif
	...
	if (flags & OPT_login) {
		////NSLog((@"[hus] login shell!");
		FILE *input;
		/*
		////NSLog((@"sourcing /etc/profile\n");
	...
	////NSLog((@"[hus] check argv");
	if (G.global_argv[1]) {
		////NSLog((@"[hus] check argv true");
		FILE *input;
		/*
	 ...
	 */
////NSLog((@"[hush] critical: isatty? %d %d", isatty(STDIN_FILENO), isatty(STDOUT_FILENO));
#if ENABLE_HUSH_JOB
	 ////NSLog((@"[hush] job: isatty? %d %d", isatty(STDIN_FILENO), isatty(STDOUT_FILENO));
	if (isatty(STDIN_FILENO) && isatty(STDOUT_FILENO)) {
		G_saved_tty_pgrp = getpid(); // XXX tcgetpgrp(STDIN_FILENO);
		//NSLog((@"[hush] saved_tty_pgrp:%d\n", G_saved_tty_pgrp);
...
// to (inadvertently) close/redirect G_interactive_fd
	...
	if (G_interactive_fd) {
		...
		if (G_saved_tty_pgrp) {
			...
			while (1) {
				pid_t shell_pgrp = getpid(); // XXX getpgrp();
				G_saved_tty_pgrp = getpid(); // XXX tcgetpgrp(G_interactive_fd);
...
# if ENABLE_HUSH_SAVEHISTORY && MAX_HISTORY > 0
		{
			...
			if (!hp) {
				...
				if (hp)
					hp = concat_path_file(hp, "/Documents/.hush_history");
			} else {
				hp = xstrdup(hp);
			}
			if (hp) {
...
# endif
	...
	////NSLog((@"[hush] interactive: isatty? %d %d", isatty(STDIN_FILENO), isatty(STDOUT_FILENO));
	...
	////NSLog((@"[hush] G_interactive_fd = %d", G_interactive_fd);
	if (G_interactive_fd) {
		////NSLog((@"[hush] close_on_exec_on");
		close_on_exec_on(G_interactive_fd);
	}
	////NSLog((@"[hush] install handlers");
	install_special_sighandlers();
	////NSLog((@"[hush] installed");
#else
	...
	////NSLog((@"[hush] parse_and_run_file");
	parse_and_run_file(stdin);
	////NSLog((@"[hush] parse_and_run_file done");
